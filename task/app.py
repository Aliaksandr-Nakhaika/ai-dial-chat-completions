...
async def start(stream: bool) -> None:
    ...
    an_client = DialClient(
        deployment_name='gpt-4o',
    )
    # 1.2. Create DialClient
    an_custom_client = DialClient(
        deployment_name='gpt-4o',
    )
    ...
    while True:
        ...
        print("AI:")
        if stream:
            ai_message = await an_custom_client.stream_completion(conversation.get_messages())
        else:
            ai_message = an_custom_client.get_completion(conversation.get_messages())
